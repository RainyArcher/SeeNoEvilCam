            ...
            if (await CameraView.RequestPermissions(true, true))
            {
                ...
                if (cameraView.Camera != null)
                {
                    try
                    {
                        ...
                        int rotation = (int)windowManager.DefaultDisplay.Rotation - ((cameraView.Camera.Position == CameraPosition.Front) ? 180 : 0);
                        if (rotation < 0)
                        {
                            rotation = (360 + rotation + 90) % 360;
                        }
                        int orientation = cameraView.Camera.Position == CameraPosition.Front ? (int)Math.Round(rotation / 10.0) * 10 : ORIENTATIONS.Get(rotation);
